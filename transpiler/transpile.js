...
import { createClass, generateTokenRegex, parseComponent, parseSystem, printFile, readTSFile, } from './shared.js';
if (false) {
    ...
    const generateTutorial = (tutorial) => {
        ...
        fs.writeFileSync(createTutorialDir('data.json'), JSON.stringify(genTutorial, null, '\t'));
...
const newlineSpacer = ts.factory.createNotEmittedStatement(ts.factory.createIdentifier('__'));
const c = createClass(fileData);
const output = printFile([
    ...c.imports,
    ts.addSyntheticLeadingComment(newlineSpacer, ts.SyntaxKind.SingleLineCommentTrivia, '', true),
