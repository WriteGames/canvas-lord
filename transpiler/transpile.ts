...
const generate = (
	...
	const componentPropertyMap = new Map<string, [string, string][]>();
	...
	rawComponents.forEach(({ groups }) => {
		...
		if (!name || !data) return;
		const properties = [...data.matchAll(regexProperty)].map<
			[string, string]
		>((m) => [m.groups?.key!, JSON.parse(m.groups?.value!)]);
	...
	rawSystems.forEach(({ groups }) => {
		...
		const { name, update, updateArgs, updateCode, render, renderArgs } =
			groups;
		if (
			!name ||
			!update ||
	...
	const createEntity = (
		...
		const consolidateSystemItems = (type: SystemDataType) => {
			return systems
