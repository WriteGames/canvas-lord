...
export const runTranspile = async ({
	jsonFilePath,
	outDir,
	...
	const oldImportFix = false as boolean;
	const printClass = false as boolean;
	...
	if (wholeThang) {
		...
		const cleanUpClass = (_class: tsm.ClassDeclaration): void => {
			[..._class.getConstructors(), ..._class.getMethods()].forEach(
				(m) => {
					// Remove empty method
					...
					// `entity` -> `this`
					m.getDescendantsOfKind(SyntaxKind.Identifier).forEach(
						(id) => {
		...
		type ComponentsMap = Map<string, tsm.ExportedDeclarations>;
		type SystemsMap = Map<string, tsm.ExportedDeclarations>;
		const getComponentProperties = (
			foundComponents: ComponentsMap,
			components: string[],
		): Array<tsm.OptionalKind<tsm.PropertyDeclarationStructure>> => {
			return components
				.flatMap((name) => {
					...
					return obj
						...
						.filter(
							(n) =>
								n.getKind() === SyntaxKind.PropertyAssignment,
						)
						.map((node) => {
		...
		const addComponents = (
			...
			getComponentProperties(foundComponents, components).forEach(
				(prop) => {
					if (firstMethodIndex) {
		...
		const addToMethod = (
			target: tsm.ClassDeclaration,
			method: tsm.MethodDeclaration | tsm.ConstructorDeclaration,
			options: System,
			block: tsm.Block,
		): void => {
		...
		const removeFromMethod = (
			...
			if (options.outputType === 'inline') {
				...
				let toRemove: tsm.Statement | undefined;
				const getNextStmt = (): tsm.Statement | undefined =>
					method.getStatements().find((s) => {
			...
			systemsToUse.forEach((system) => {
				...
				methods.forEach((m) => {
					...
					let method:
						...
						| tsm.ConstructorDeclaration
						| undefined = target.getMethod(methodName);
		...
		if (transpileProjectIntoTS) {
			...
			const foundComponents: ComponentsMap = new Map();
			const foundSystems: SystemsMap = new Map();
			...
			const sourceFiles = steps.map((step, i) => {
				...
				const sourceFile = project.createSourceFile(
					playerOutputPath.replace('.ts', `.${ext}.ts`),
					'',
					{
						overwrite: true,
					},
		...
		if (runEslint) {
			...
			const eslint = new ESLint({
				fix: true,
				overrideConfigFile:
					'C:\\xampp\\apps\\canvas-lord\\transpiler\\.eslintrc.cjs',
			});
